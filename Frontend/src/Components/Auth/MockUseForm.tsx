...
interface UseFormReturn {
  register: (name: string, validation?: unknown) => {
    name: string;
...
const MockUseForm = (config?: UseFormConfig): UseFormReturn => {
  ...
  const register = (name: string) => ({
    ...
    onBlur: () => {
      if (config?.resolver && formData[name as keyof FormData]) {
        const mockValidation = validateField(name, formData[name as keyof FormData] || '');
        if (mockValidation) {
  ...
  const watch = (field: string): string => formData[field as keyof FormData] || '';
