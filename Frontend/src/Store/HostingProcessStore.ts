...
interface HostingProcessState {
  ...
  setWeekDayPricing: (pricing: number) => void;
  setWeekendPricing: (pricing: number) => void;
...
export const useHostingProcessStore = create<HostingProcessState>(
  (set, get) => ({
    // Initial state
    address: null,
    coordinates: null,
    ...
    setHighlight: (highlight) =>
      set((state) => ({
        listingInfo: {
          ...state.listingInfo,
          highlight: state.listingInfo.highlight.includes(highlight)
            ? state.listingInfo.highlight.filter((h) => h !== highlight)
            : state.listingInfo.highlight.length < 2
            ? [...state.listingInfo.highlight, highlight]
            : state.listingInfo.highlight,
        },
      })),
    ...
    setAmenities: (amenities) =>
      set((state) => ({
        listingInfo: {
          ...state.listingInfo,
          amenities: [...state.listingInfo.amenities, amenities],
        },
      })),
    ...
    removeAmenity: (amenities) =>
      set((state) => ({
        listingInfo: {
          ...state.listingInfo,
          amenities: state.listingInfo.amenities.filter(
            (amenity) => amenity !== amenities
          ),
        },
      })),
    setWeekDayPricing: (pricing) =>
      set((state) => ({
        listingInfo: {
          ...state.listingInfo,
          pricing: { ...state.listingInfo.pricing, weekdayPrice: pricing },
        },
      })),

    setWeekendPricing: (pricing) =>
      set((state) => ({
        listingInfo: {
          ...state.listingInfo,
          pricing: { ...state.listingInfo.pricing, weekendPrice: pricing },
        },
      })),
    addImage: (image) =>
      set((state) => ({
      ...
      switch (step) {
        ...
        case 2: // Choose Property Type
          return {
            ...
            requiredFields: state.listingInfo.propertyType
              ? []
              : ["propertyType"],
          };
        ...
        case 3: // Type of Place
          return {
            ...
            requiredFields: state.listingInfo.typeOfPlace
              ? []
              : ["typeOfPlace"],
          };
        ...
        case 4: { // Address
          return {
            isValid:
              ...
              !isNaN(state.coordinates.lat) &&
              !isNaN(state.coordinates.lng),
        ...
        case 7: // Choose Amenities
          return {
            ...
            requiredFields:
              state.listingInfo.amenities.length > 0 ? [] : ["amenities"],
          };
        ...
        case 10: // Property Title
          return {
            isValid:
              ...
              state.listingInfo.title.trim().length > 0,
            requiredFields:
              !state.listingInfo.title ||
              state.listingInfo.title.trim().length === 0
                ? ["title"]
        ...
        case 11: // Property Description/Tag
          return {
            isValid:
              ...
              state.listingInfo.description.trim().length > 0,
            requiredFields:
              !state.listingInfo.description ||
              state.listingInfo.description.trim().length === 0
                ? ["description"]
        ...
        case 12: // Property HighLight
          return {
            isValid: state.listingInfo.highlight.length > 0,
            requiredFields:
              state.listingInfo.highlight.length === 0 ? ["highlight"] : [],
          };

        case 13: // Finish up and publish intro - always valid
          return { isValid: true, requiredFields: [] };

        case 14: // Weekday Base Price
          return {
            isValid:
              !!state.listingInfo.pricing.weekdayPrice &&
              state.listingInfo.pricing.weekdayPrice > 0,
            requiredFields:
              !state.listingInfo.pricing.weekdayPrice ||
              state.listingInfo.pricing.weekdayPrice <= 0
                ? ["weekdayPrice"]
        ...
        case 15: // Weekend Price
          return {
            isValid:
              !!state.listingInfo.pricing.weekendPrice &&
              state.listingInfo.pricing.weekendPrice > 0,
            requiredFields:
              !state.listingInfo.pricing.weekendPrice ||
              state.listingInfo.pricing.weekendPrice <= 0
                ? ["weekendPrice"]
